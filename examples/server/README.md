...
The WebSocket server example (`examples/server`) provides real-time audio transcription via WebSocket connections. It supports both binary audio data and JSON configuration messages.

#### Building

```bash
# Build with WebSocket support (requires Boost)
cmake -B build -DWHISPER_SUPPORT_BOOST=ON
cmake --build build

# Run the server
./build/bin/whisper-server -m models/ggml-base.en.bin --step 0 --length 30000
```

#### Audio Processing Configuration

The server supports configurable audio processing parameters that can be set via command line or JSON messages:

Command line options:
```bash
--st, --silence-threshold    Silence detection threshold in dB (default: -40.0)
--mal, --min-audio-level    Minimum audio level in dB (default: -60.0)
--sd, --silence-duration    Duration of silence to trigger processing in seconds (default: 0.5)
--mad, --min-audio-duration Minimum duration of audio to process in seconds (default: 2.0)
```

Client-side configuration via JSON:
```json
{
    "format": "pcm16",           // Audio format: "pcm16" or "float32"
    "silence_threshold": -35.0,  // Silence detection threshold in dB
    "min_audio_level": -55.0,   // Minimum audio level in dB
    "silence_duration": 0.3,     // Duration of silence to trigger processing
    "min_audio_duration": 1.5    // Minimum duration of audio to process
}
```

The server responds with the current configuration:
```json
{
    "status": "ok",
    "format": "pcm16",
...
When audio processing is triggered, the server responds with the transcription:
```json
{
...
```bash
sudo apt-get install libboost-system-dev libboost-thread-dev libssl-dev zlib1g-dev
```
